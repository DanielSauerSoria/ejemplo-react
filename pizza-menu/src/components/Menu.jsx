...
export default function Menu() {
    ...
    //este hook es lanzado cuando se construye el componente
    useEffect(() => {
        const data = localStorage.getItem("pizzas");
        if(data) setPizzas(JSON.parse(data));
    }, []);

    //fijas los datos o actualizarlos
    ...
    return (
        <main className="menu">
            <h2>Our menu</h2>
            {pizzas.length > 0 ?
                //esta etiqueta se conoce como fragment, no se renderiza en la aplicacion
                (<>
                    <p>
